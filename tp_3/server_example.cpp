...
void *worker(void *arg)
{
	...
	while (!done) {
		...
		if(receveidMessageType == 1){
			sleep(2); // Apenas teste. Finge que houve demora até a fila estar vazia e chegar a vez desse cliente.
			string grantMessage = createMessage(2, receveidProcessID);
			fprintf(write_fh, "%s\n", grantMessage.c_str());
			fflush(write_fh);
            // if(requests.empty()){
            //     string grantMessage = createMessage(2, receveidProcessID);
            //     sendto(sockfd, grantMessage.c_str(), size(grantMessage), MSG_CONFIRM, (const struct sockaddr *) &cliaddr, sizeof(servaddr));
            // }
            // requests.push(receveidProcessID);
			
        }
        if(receveidMessageType == 3){
            // requests.pop();
            // int nextProcess;
            // if(! requests.empty()){
            //     nextProcess = requests.front();
            //     string grantMessage = createMessage(2, nextProcess);
            //     sendto(sockfd, grantMessage.c_str(), size(grantMessage), MSG_CONFIRM, (const struct sockaddr *) &cliaddr, sizeof(servaddr));
            // }
